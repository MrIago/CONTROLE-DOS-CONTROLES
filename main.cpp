...
int r1x0 = 10;
int r1x1 = (sx-10)/3-2.5;

int r2x0 = (sx-10)/3+2.5;
int r2x1 = 2*((sx-10)/3)-2.5;

int r3x0 = 2*((sx-10)/3)+2.5;
int r3x1 = sx-10;
...
rectangle(0, 100, sx, sy-70);
floodfill(sx/2, sy/2, LIGHTGRAY);
...
// CAIXA DE INFO
rectangle(10, sy-145, sx-10, sy-115);
floodfill(11, sy-140, BLACK);

// CAIXA DE ACAO
rectangle(10, sy-110, 404, sy-80);
floodfill(11, sy-109, BLACK);

// CAIXA DE TEXTO
rectangle(407, sy-110, sx-10, sy-80);
floodfill(408, sy-109, BLACK);
...
settextstyle(10, 0, 5);
char title[30] = "CONTROLE DOS CONTROLES";
int tx = textwidth(title);
outtextxy(mx-tx/2, 35, title);

//CLUBE DE PROGRAMACAO
settextstyle(10, 0, 1);
char title2[30] = "CLUBE DE PROGRAMACAO";
tx = textwidth(title2);
outtextxy(5, sy-50, title2);

//CLUBE DE PROGRAMACAO
settextstyle(10, 0, 1);
char title3[30] = "IFF MACAE";
tx = textwidth(title3);
outtextxy(sx-tx-5, sy-50, title3);


//RETANGULO CONTROLE
setcolor(BLACK);
setfillstyle(1, BLACK);
rectangle(10, 110, (sx-10)/3-2.5, sy-150);
floodfill(11, 111, BLACK);

//RETANGULO CONTROLE
rectangle((sx-10)/3+2.5, 110, 2*((sx-10)/3)-2.5, sy-150);
floodfill((sx-10)/3+6, 111, BLACK);

//RETANGULO CONTROLE
rectangle(2*((sx-10)/3)+2.5, 110, sx-10, sy-150);
floodfill(2*((sx-10)/3)+6, 111, BLACK);

//RETANGULO CONTROLE - TITULO
setcolor(DARKGRAY);
setfillstyle(1, DARKGRAY);
rectangle(10, 110, (sx-10)/3-2.5, 110+2*ly);
floodfill(11, 112, DARKGRAY);

//RETANGULO CONTROLE - TITULO
rectangle((sx-10)/3+2.5, 110, 2*((sx-10)/3)-2.5, 110+2*ly);
floodfill((sx-10)/3+6, 112, DARKGRAY);

//RETANGULO CONTROLE - TITULO
rectangle(2*((sx-10)/3)+2.5, 110, sx-10, 110+2*ly);
floodfill(2*((sx-10)/3)+6, 112, DARKGRAY);
...
int m1 = ((r1x1-r1x0)/2);
int m2 = ((r2x1-r2x0)/2)+r2x0;
int m3 = ((r3x1-r3x0)/2)+r3x0;

int Y = y0-textheight(t1);

outtextxy(m1-textwidth(t1)/2, Y, t1);
outtextxy(m2-textwidth(t2)/2, Y, t2);
outtextxy(m3-textwidth(t3)/2, Y, t3);
...
void creditos(){
    ...
    "Clube de Programação - IFF Macaé",
    "CONTROLE DOS CONTROLES",
    "",
    "",
    "A versão original desse programa foi desenvolvida por:",
    "",
    "Pedro Henrique de Souza Guimarães",
    "Renan Macchirulo",
    "Willian Bittencourt Coelho",
    "",
    "Professor Coordenador: Lucas Scotta",
    "",
    "Feita em C Puro, como nas aulas de ATP!",
    "",
    "",
    "Essa é uma nova versão do programa,",
    "com a implementação da Interface Gráfica e do sistema de controle de chaves",
    "para bolsistas e servidores autorizados.",
    "",
    "Desenvolvida por: Iago Lima Toledo",
    "",
    "",
    "CLUBE DE PROGRAMAÇÃO - IFF CAMPUS MACAÉ",
    "",
    "Reunião todas as terças-feiras, no Lab de BD, às 15:35",
    "PASSA LÁ! :)",
    "",
    "cprogramacao.iffmacae@gmail.com",
    "",
    "",
    "Pressione qualquer tecla para voltar a tela inicial..."
    ...
    settextstyle(8, 0, 1);
    setcolor(LIGHTBLUE);
    outtextxy(100, sy/2-lsizey/2, texto[0]);
    ...
    while(!kbhit() && s > 3){
        ...
        cleardevice();
        settextstyle(10, 0, s);
        outtextxy(mx-textwidth(texto[1])/2, my, texto[1]);
        s--;
        delay(777);
    ...
    outtextxy(mx-textwidth(texto[1])/2, my, texto[1]);
    settextstyle(10, 0, 1);
    for(i = 2; i < 31; i++) outtextxy(mx-textwidth(texto[i])/2, y+lsizey*2*i, texto[i]);
...
if(strcmp(matricula, "1357") == 0) return 1;
...
if(tela >= 0 && tela < n) {
    ...
    outtextxy(c_x_m_a-textwidth(acoes[tela])/2, c_y_a, acoes[tela]);
